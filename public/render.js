...
const allActions = [];
const baseActions = {
    StandingPose: { weight: 1 },
    TipToe: { weight: 0 },
    Running: { weight: 0 },
    DropKick: { weight: 0},
    Running: { weight: 0},
    FlipKick: { weight: 0},
    SkinningTest: { weight: 0},
    HurricaneKick: { weight: 0},
    NorthernSoulSpinCombo: { weight: 0},
    HipHop: { weight: 0},
};
...
function init() {
    ...
    const loader = new GLTFLoader();
  	loader.load('https://www.dl.dropboxusercontent.com/s/ieu7fv5g5ko7fjk/char_web.glb?dl=1', function (gltf) {
...
function prepareCrossFade(startAction, endAction, duration) {
    // If the current action is 'StandingPose', execute the crossfade immediately;
    // else wait until the current action has finished its current loop
    if (currentBaseAction === 'StandingPose' || !startAction || !endAction) {
